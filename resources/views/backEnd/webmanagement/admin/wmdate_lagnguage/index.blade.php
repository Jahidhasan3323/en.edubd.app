...
@section('content')
    <div class="panel col-sm-12" style="margin-top: 15px; margin-bottom: 15px;">
        <div class="page-header">
            <h1 class="text-center text-temp">Date Language </h1>
        </div>
            ...
            <div class="col-sm-12 " style="font-size: 18px; box-shadow: 0 2px 5px 0 rgba(0, 0, 0, 0.16), 0 2px 10px 0 rgba(0, 0, 0, 0.12); padding: 30px">
                ...
                @if($date_languages)
                    ...
                    @foreach($date_languages as $date_language)
                        <tr>
                            <td>{{$i++}}</td>
                            <td>{{$date_language->date_language->tittle}}</td>


                            <td>
                                <a style="margin-bottom: 10px;" href="{{url('/date_language/edit/'.$date_language->id)}}" class="btn btn-info"><span class="glyphicon glyphicon-edit"></span></a>

                               <a style="margin-bottom: 10px" href=""  onclick="event.preventDefault(); clickFunction{{$date_language->id}}()"
                                       class="btn btn-danger"><span class="glyphicon glyphicon-trash"></span>
                                    </a>
                                <form style="display: none;" id="delete-form{{$date_language->id}}" method="post" action="{{url('/date_language/delete/'.$date_language->id)}}" style="padding: 0; margin: 0; outline: 0;">
                                    {{method_field('delete')}}
                                    {{csrf_field()}}
                                </form>
                                <script>
