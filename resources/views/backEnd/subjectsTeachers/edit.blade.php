...
@section('content')
    <div class="panel col-sm-12" style="margin-top: 15px; margin-bottom: 15px;">
        ...
        <div class="panel-body">
            <form action="{{url('/subjectTeachers/update',[$subjectTeacher->id])}}" method="post" enctype="multipart/form-data">
                {{csrf_field()}}
                <div class="row">
                    <div class="col-sm-6">
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group {{$errors->has('subject_id') ? 'has-error' : ''}}">
                            ...
                            @endif
                                {{isset($old_sb_t->subject_name)?$old_sb_t->subject_name:$subjectTeacher->subject->subject_name}}</option>
                                </select>
                            </div>
...
@endsection
 ...
 @else
    ...
    @if(isset($subjectTeacher))
    <script type="text/javascript">
    document.getElementById('staff_id').value="{{$subjectTeacher->staff_id}}";
    document.getElementById('master_class_id').value="{{$subjectTeacher->master_class_id}}";
    document.getElementById('shift').value="{{$subjectTeacher->shift}}";
    document.getElementById('section').value="{{$subjectTeacher->section}}";
    document.getElementById('group_class_id').value="{{$subjectTeacher->group_class_id}}";
    </script>
    @endif
...
<script>
    $('#group_class_id').on('change', function () {
        ...
        var group_class_id = $(this).val();
            var option = '<option value="">... বিষয় নির্বাচন করুন ...</option>';
            $.ajax({
                url : "{{url('subjectTeachers/getsubject')}}",
                type: 'get',
                data: {
