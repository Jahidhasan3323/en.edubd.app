...
@section('content')
    <div class="panel col-sm-12" style="margin-top: 15px; margin-bottom: 15px;">
        <div class="page-header">
            <h1 class="text-center text-temp">Class Routine</h1>
        </div>
                        ...
                        <table class="table table-bordered table-responsive table-hover">
                            <tr>
                                <td>
                                    ...
                                    <a class="{{($status==0) ? 'text-danger':'text-success'}}" target="_blank" href="{{Storage::url($routine->path)}}">&nbsp;{{$routine->master_class->name.' Class-'.$routine->name.' ( Published '.$routine->updated_at->format('d-m-Y').' )'}}</a>
                                </td>
                                @if (Auth::is('admin'))
                                <td>
                                        @if($status==0)
